...
class ConvexQuadraticConstraint:
    ...
    def asCvxpy(self, y, epsilon=0.0):
        return [
            0.5 * cp.quad_form(y, self.P, assume_PSD=True) + self.q.T @ y + self.r
            <= -epsilon
...
class ConvexConstraints:
    ...
    def __init__(
        ...
        self.k = all_dim[0]
        self.m = input_dim  # Constraint data depends on this input x
        ...
        if self.has_linear_constraints:
            ...
            if do_preprocessing_linear:
                ...
                if A.shape[0] > 1:  # If there is more than one constraint
                    ...
                    for i in tqdm(reversed_indexes):
                        ...
                        constraints = [
                            A[all_rows_but_i, :] @ z <= b[all_rows_but_i, :],
                            A[i, :] @ z <= (b[i, 0] + 1),
                        ]
                        ...
                        if (
                            prob.status != "optimal"
                            and prob.status != "optimal_inaccurate"
                        ):
                    ...
                    if print_debug_info:
                        utils.printInBoldBlue(
                            f"Removed {len(indexes_const_removed)} constraints "
                        )
                        utils.printInBoldGreen(f"A is {A.shape} and b is {b.shape}")
                    ################################################
                ...
                for i in tqdm(range(A.shape[0])):
                    objective = cp.Minimize(
                        A[i, :] @ z - b[i, 0]
                    )  # I try to go far from the constraint, into the feasible set
                    constraints = [A @ z <= b]
                    prob = cp.Problem(objective, constraints)
                    if self.solver == "GUROBI":
                        result = prob.solve(
                            verbose=False, solver=self.solver, reoptimize=True
                        )
                    else:
                        TOL = 1e-5
                        result = prob.solve(
                            verbose=False, solver=self.solver
                        )  # When using Gurobi, we need the reoptimize parameter because if not the solver cannot distinguish between infeasible or unbounded. This is the error you get:
                    ...
                    utils.verify(
                        obj_value < TOL,
                        f"The objective should be negative. It's {obj_value} right now",
                    )

                    if (
                        obj_value > -TOL
                    ):  # if the objective value is zero (I tried to go far from the constraint, but I couldn't)
                        E.append(i)
            ...
            A_E = np.zeros((1, self.k))
            # 0y=0
            b_E = np.zeros((1, 1))

            A_I = np.zeros((1, self.k))
            # 0y<=1
            b_I = np.ones((1, 1))
        ...
        objective = cp.Minimize(
            cp.sum_squares(self.y_projected - self.y_to_be_projected)
        )
    ...
    def getConstraintsInSubspaceCvxpy(self, z, epsilon=0.0):
        constraints = [
            self.A_p @ z - self.b_p <= -epsilon * np.ones((self.A_p.shape[0], 1))
        ]
    ...
    def project(self, y_to_be_projected):
        ...
        if (
            self.prob_projection.status != "optimal"
            and self.prob_projection.status != "optimal_inaccurate"
        ):
            raise Exception(
                f"Value is not optimal, prob_status={self.prob_projection.status}"
            )
    ...
    # TODO
    # Check if y is interior point
    def isInteriorPoint(self, y, epsilon=0.0):
        constraints = self.getConstraintsCvxpy(y, epsilon)
        if np.alltrue(constraints):
            return True
        else:
            return False

    # TODO
    # Update constraint data within this class
    def updateConstraintSet(self, x):
        # Custom map from x to constraint data
        self.customConstraintMap(x)

        # Update all necessary parameters as below
    ...
    # TODO
    # Problem-specific map from input x to constraint data
    def customConstraintMap(self, x):
