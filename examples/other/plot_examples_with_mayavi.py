...
dist_min = 0
dist_max = 2
all_x, all_y, all_z = np.mgrid[
    -dist_min:dist_max:num_points,
    -dist_min:dist_max:num_points,
    -dist_min:dist_max:num_points,
]
...
result = Parallel(n_jobs=40)(
    delayed(my_function)(i, j, k, conditions)
...
for i in range(len(conditions)):
    ...
    for j in range(len(conditions)):
        ...
        if cond_j_is_ineq:
            cond_i[
                cond_j > 0.0
            ] = None  # See https://stackoverflow.com/questions/40461045/mayavi-combining-two-implicit-3d-surfaces
        else:
...
for i in range(len(conditions_processed)):
    ...
    tmp = mlab.contour3d(
        all_x, all_y, all_z, cond_i, contours=[0], color=color, opacity=1.0
    )
