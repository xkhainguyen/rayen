...
fig.suptitle("polyhedron + circle", fontsize=14)
...
for i in range(num_cstr_samples):
    ...
    xv_batched_x = torch.Tensor(num_samples, 1).uniform_(-2, 2) * 1
    xv_batched_y = torch.Tensor(num_samples, 1).uniform_(-2, 2) * 1
    if example.xv_dim == 3:
        xv_batched_z = torch.Tensor(num_samples, 1).uniform_(-2, 2) * 1
