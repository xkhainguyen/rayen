...
results2d["[In dist] loss"] /= results2d.loc["dataset2d_Optimization"].at[
    "[In dist] loss"
]  # Normalize the cost
results2d["[Out dist] loss"] /= results2d.loc["dataset2d_Optimization"].at[
    "[Out dist] loss"
]  # Normalize the cost

results3d["[In dist] loss"] /= results3d.loc["dataset3d_Optimization"].at[
    "[In dist] loss"
]  # Normalize the cost
results3d["[Out dist] loss"] /= results3d.loc["dataset3d_Optimization"].at[
    "[Out dist] loss"
]  # Normalize the cost
...
df = df.rename(
    columns={
        "[In dist] loss": "[In dist] n.loss",
        "[Out dist] loss": "[Out dist] n.loss",
    }
)
