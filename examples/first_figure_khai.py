...
my_layer = constraint_module.ConstraintModule(
    constraint, method=method, create_map=False
)

num_samples = 1000  # 12000
v_batched_x = torch.Tensor(num_samples, 1, 1).uniform_(-2.5, 2.5)
v_batched_y = torch.Tensor(num_samples, 1, 1).uniform_(-2.5, 2.5)
v_batched_z = torch.Tensor(num_samples, 1, 1).uniform_(-2.5, 2.5)
